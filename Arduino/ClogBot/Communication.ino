...
void checkForCom()
{
  if (Serial.available() > 0) {
    byte startBit = Serial.read();
    if (startBit == 0xFF) {
      // Then we're doing the joint controls!
      getJointAngles();
    }
    else if (startBit == 0xFD) {
      // Then we're doing a face expression!
      getEmote();
    }
    else
    {
...
void getEmote() {
  delay(10);  // Wait a bit for all bytes to arrive this was needed for it to work
      //int angles[20];
      int byteCount = 0;
      bool endByteReceived = false;

      // Read up to 2 bytes, where one is the emote byte, and the other is hopefully the end byte
    while (Serial.available() > 0 && byteCount <= 1) { // Ensure we do not overflow the jointAngles array
        int byteRead = Serial.read();
      ...
      if (endByteReceived) {
        ...
        Serial.println("Incomplete or incorrect emote packet");
      }
